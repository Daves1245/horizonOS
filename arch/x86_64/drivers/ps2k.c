	...
	if (released) {
		...
		if (index == 0x2A || index == 0x36) {
			...
			if (c != 0) {
				...
				for (int lvl = 0; lvl < KEYBOARD_QUEUE_LEVELS;
				     lvl++) {
...
static int ps2k_create_device(void) {
	log_info("[ps2k] IRQ=%d data=0x%x cmd=0x%x\n", ps2k_resources.irq,
		 ps2k_resources.port_data, ps2k_resources.port_cmd);
...
uacpi_iteration_decision ps2k_handle_resource(void *user,
...
static int ps2k_probe(uacpi_namespace_node *node,
	...
	uacpi_for_each_resource(kb_res, ps2k_handle_resource,
				(void *)&ps2k_resources);
...
static struct acpi_driver ps2k_driver = { .device_name = "ps2 Keyboard",
