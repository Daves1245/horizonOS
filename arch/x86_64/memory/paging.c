	...
	if (!(pue_val(*pue) & PAGE_PRESENT)) {
		...
		*pue_ptr(pue) = pd_phys | PAGE_PRESENT | PAGE_WRITE;
		...
		if (pue_val(*pue) & PAGE_PS) {
			...
			phys_addr_t split =
				split_huge_entry(pue_val(*pue), 0x200000UL);
	...
	if (!(pde_val(*pde) & PAGE_PRESENT)) {
		...
		*pde_ptr(pde) = pt_phys | PAGE_PRESENT | PAGE_WRITE;
		...
		if (pde_val(*pde) & PAGE_PS) {
			...
			phys_addr_t split =
				split_huge_entry(pde_val(*pde), PAGE_SIZE);
 ...
 */
void map_page(virt_addr_t virt_addr, uint64_t phys_addr, int iskernel,
	...
	if (!pte) {
		log_error(
			"[paging]: Failed to get page entry for vaddr 0x%x%x\n",
