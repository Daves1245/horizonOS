...
void kernel_main(void) {
	...
	for (uint64_t i = 0; i < memmap->entry_count; i++) {
		...
		if (bump_phys == 0 && e->type == LIMINE_MEMMAP_USABLE &&
		    e->length >= 0x400000) {
			bump_phys = e->base;
		}
	}
	...
	if (framebuffer_request.response == NULL ||
		...
		serial_write(
			"[kernel.c]: did not receive a framebuffer from limine. exiting\n");
		hcf();
	}
	...
	map_physical_range(rsdp_phys, 4096, 1, 1,
			   cr3); // Map 4KB, kernel, writable
	...
	log_info("local APIC: phys=0x%x virt=0x%x%x\n", lapic_phys,
		 (uint32_t)(lapic_virt >> 32), (uint32_t)lapic_virt);

	map_physical_range(ioapic_phys, 4096, 1, 1, cr3);
	ioapic_addr = (void *)(hhdm_offset + ioapic_phys);
	log_info("I/O APIC: phys=0x%x virt=0x%x%x\n", ioapic_phys,
		 (uint32_t)((uintptr_t)ioapic_addr >> 32),
		 (uint32_t)(uintptr_t)ioapic_addr);
	...
	extern struct key_event_t
		keyboard_multilevel_queue[KEYBOARD_QUEUE_LEVELS]
					 [RING_BUFFER_SIZE];
	extern struct keyboard_queue_state
		keyboard_queue_state[KEYBOARD_QUEUE_LEVELS];
	...
	init_scheduler();
	// TODO(refactoring): ideally this should be init(), and init_process() bootstraps itself and
	// starts the scheduler.
	//
	// note that init_process() does not return and we switch into a different execution
	// context.
	init_process();

	panic("kernel_main: returned from init_process()");
}
