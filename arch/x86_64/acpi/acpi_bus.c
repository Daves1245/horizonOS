...
static uacpi_iteration_decision acpi_init_one_device(void *ctx,
	...
	if (info->flags & UACPI_NS_NODE_INFO_HAS_HID) {
		for (struct acpi_driver *driver_it = acpi_drivers_head;
		     driver_it; driver_it = driver_it->next) {
			for (const char *const *id = driver_it->pnp_ids; *id;
			     id++) {
	...
	if (!driver && (info->flags & UACPI_NS_NODE_INFO_HAS_CID)) {
		...
		for (struct acpi_driver *driver_it = acpi_drivers_head;
			...
			for (uint32_t i = 0; i < info->cid.num_ids; i++) {
				for (const char *const *id = driver_it->pnp_ids;
				     *id; id++) {
					if (strcmp(*id,
