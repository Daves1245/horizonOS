...
void kernel_main(uint32_t multiboot_info_p) {
	...
	struct multiboot_info *mb_info =
		(struct multiboot_info *)multiboot_info_p;
	...
	if (mb_info->flags & MB_FLAG_FRAMEBUFFER) {
		...
		log_info(
			"[multiboot]: type=%d addr=0x%x pitch=%d %dx%d bpp=%d\n",
		...
		log_warn(
			"[multiboot]: no framebuffer info — vbe request likely ignored by grub\n");
	}
	...
	if (ioapic_addr == 0) {
		...
		configure_ioapic_irq_with_flags(kbd_gsi, 33, local_apic_id,
						kbd_flags);
		log_info(
			"[kernel]: keyboard interrupt configured via IOAPIC\n");
	}
