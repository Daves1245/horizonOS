...
void terminal_putentryat_visual_debug(unsigned char c, uint8_t color, size_t x,
				      size_t y) {
	if (x >= VGA_WIDTH || y >= VGA_HEIGHT) {
		// Flash corner of screen bright red to indicate bounds error
		terminal_buffer[0] = vga_entry(
...
void terminal_putentryat_interrupt_debug(unsigned char c, uint8_t color,
					 size_t x, size_t y) {
	if (x >= VGA_WIDTH || y >= VGA_HEIGHT) {
