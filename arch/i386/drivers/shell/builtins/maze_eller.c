...
void generate_maze_eller() {
	...
	for (int y = 0; y < MAZE_HEIGHT - 1; y++) {
		...
		for (int x = 0; x < MAZE_WIDTH - 1; x++) {
			...
			if (maze_eller[y][x].set_id !=
			    maze_eller[y][x + 1].set_id) {
				...
				if (random_range(100) < 50) {
					...
					for (int i = 0; i < MAZE_WIDTH; i++) {
						if (maze_eller[y][i].set_id ==
						    old_set) {
							maze_eller[y][i].set_id =
								new_set;
						}
					}
				}
		...
		for (int s = 0; s < num_unique_sets; s++) {
			...
			for (int x = 0; x < MAZE_WIDTH; x++) {
				if (maze_eller[y][x].set_id == current_set) {
					...
					int should_connect = random_range(100) <
							     40;

					if (should_connect || !has_connection) {
						maze_eller[y][x].bottom_wall =
							0;
						maze_eller[y + 1][x].type =
							CORRIDOR;
						maze_eller[y + 1][x].set_id =
							current_set;
						maze_eller[y + 1][x].right_wall =
							1;
						maze_eller[y + 1][x]
							.bottom_wall = 1;
						has_connection = 1;
	...
	for (int x = 0; x < MAZE_WIDTH - 1; x++) {
		if (maze_eller[last_row][x].set_id !=
		    maze_eller[last_row][x + 1].set_id) {
