...
void draw_maze() {
	...
	for (int y = 0; y < MAZE_HEIGHT; y++) {
		for (int x = 0; x < MAZE_WIDTH; x++) {
			...
			display_buffer[cell_display_y][cell_display_x] =
				cell_char;

			// 2. Draw right wall (vertical wall between this cell and cell to the right)
			if (x < MAZE_WIDTH - 1 &&
			    cell_display_x + 1 < DISPLAY_WIDTH) {
				if (maze[y][x].right_wall) {
					display_buffer[cell_display_y]
						      [cell_display_x + 1] =
							      '#';
				} else {
					...
					if (maze[y][x].type == PATH ||
					    (x + 1 < MAZE_WIDTH &&
					     maze[y][x + 1].type == PATH)) {
						wall_char = 'p';
					} else if (maze[y][x].type == VISITED ||
						   (x + 1 < MAZE_WIDTH &&
						    maze[y][x + 1].type ==
							    VISITED)) {
						wall_char = 'v';
			...
			if (y < MAZE_HEIGHT - 1 &&
			    cell_display_y + 1 < DISPLAY_HEIGHT) {
				if (maze[y][x].bottom_wall) {
					display_buffer[cell_display_y + 1]
						      [cell_display_x] = '#';
				} else {
					// No wall = corridor continues, inherit cell type from adjacent cells
					char wall_char = ' ';
	...
	for (int y = 0; y < DISPLAY_HEIGHT; y++) {
		for (int x = 0; x < DISPLAY_WIDTH; x++) {
			...
			if (ch == '#') {
				...
				color = vga_entry_color(VGA_COLOR_CYAN,
							VGA_COLOR_CYAN);
				terminal_putentryat(' ', color, x, y);
			} else if (ch == 'v') {
				// Visited - use brown background
				color = vga_entry_color(VGA_COLOR_BROWN,
							VGA_COLOR_BROWN);
				terminal_putentryat(' ', color, x, y);
			} else {
				// Corridor/space - use black background
				color = vga_entry_color(VGA_COLOR_LIGHT_GREY,
							VGA_COLOR_BLACK);
				terminal_putentryat(' ', color, x, y);
			}
		}
...
void generate_maze() {
	...
	while (stack_size > 0) {
		...
		if (num_neighbors > 0) {
			...
			if (direction ==
			    0) { // up - remove bottom wall of neighbor
				maze[next_y][next_x].bottom_wall = 0;
			} else if (direction ==
				   1) { // right - remove right wall of current
				maze[current_y][current_x].right_wall = 0;
			} else if (direction ==
				   2) { // down - remove bottom wall of current
				maze[current_y][current_x].bottom_wall = 0;
			} else if (direction ==
				   3) { // left - remove right wall of neighbor
...
int dfs_solve(int x, int y, int target_x, int target_y) {
	...
	for (int i = 0; i < 4; i++) {
		...
		if (can_move_to(x, y, next_x, next_y) &&
		    (maze[next_y][next_x].type == CORRIDOR ||
		     maze[next_y][next_x].type == SINK)) {
