...
void console_init(void) {
	...
	cell_w = console_font->FixedWidth ? console_font->FixedWidth :
					    console_font->GlyphWidth[0];
...
void console_vprintf(const char *fmt, va_list args) {
	for (const char *p = fmt; *p; p++) {
		...
		case 'd':
			...
			break;
		case 'u':
			console_write_uint((uint64_t)va_arg(args, unsigned int),
					   10);
			break;
		case 'x':
			console_write_uint((uint64_t)va_arg(args, unsigned int),
		...
		case 'p':
			console_puts("0x");
			console_write_uint(
				(uint64_t)(uintptr_t)va_arg(args, void *), 16);
			break;
		case 's':
