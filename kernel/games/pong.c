...
/* PS/2 scan code set 1; extended keys stored with bit 7 set as the arch flag */
#define KEY_W 0x11
#define KEY_S 0x1F
#define KEY_ARROW_UP 0xC8 /* extended 0x48 | 0x80 */
#define KEY_ARROW_DOWN 0xD0 /* extended 0x50 | 0x80 */
#else
...
void pong_init(int width, int height) {
	...
	player2 = (struct obj){ screen_width - PADDLE_WIDTH,
				screen_height / 2 - PADDLE_HEIGHT / 2,
				PADDLE_WIDTH,
				PADDLE_HEIGHT,
				0,
	...
	ball = (struct obj){ screen_width / 2 - BALL_RADIUS / 2,
	...
	for (int row = 0; row < 7; row++) {
		for (int col = 0; col < 5; col++) {
			if (display[digit][row][col]) {
				gfx_fill_rect(x + col * FONT_SCALE,
					      y + row * FONT_SCALE,
					      x + (col + 1) * FONT_SCALE,
					      y + (row + 1) * FONT_SCALE,
					      rgb(0xff, 0xff, 0xff));
...
void pong_draw(void) {
	...
	gfx_fill_rect(player1.x, player1.y, player1.x + player1.width,
		      player1.y + player1.height, rgb(0xff, 0xff, 0xff));
	gfx_fill_rect(player2.x, player2.y, player2.x + player2.width,
		      player2.y + player2.height, rgb(0xff, 0xff, 0xff));
	gfx_fill_rect(ball.x, ball.y, ball.x + ball.width, ball.y + ball.height,
		      rgb(0xff, 0xff, 0xff));

	draw_score(score_player1, screen_width / 4 - (5 * FONT_SCALE) / 2, 20);
	draw_score(score_player2, 3 * screen_width / 4 - (5 * FONT_SCALE) / 2,
		   20);

	for (int i = 0; i < screen_height; i += 2 * NET_HEIGHT) {
		gfx_fill_rect(screen_width / 2 - NET_WIDTH / 2, i,
			      screen_width / 2 + NET_WIDTH / 2, i + NET_HEIGHT,
			      rgb(0xff, 0xff, 0xff));
	}
}
