...
void parse_madt(void *madt_addr) {
	...
	uint8_t *entry =
		(uint8_t *)madt_addr + sizeof(struct madt_extended_header);
	uint8_t *end = (uint8_t *)madt_addr + length;

	while (entry < end) {
		struct madt_record_header *entry_header =
			(struct madt_record_header *)entry;
		...
		case 0: { // processor Local APIC
			...
			log_debug(
				"[madt::parse_madt]: Local APIC: Processor ID=%d, APIC ID=%d, Flags=0x%x\n",
			...
			struct ioapic *io_apic =
				(struct ioapic
					 *)(entry +
					    sizeof(struct madt_record_header));
			log_debug(
				"[madt::parse_madt]: I/O APIC: ID=%d, Address=0x%x, IRQ Base=%d\n",
				io_apic->ioapic_id, io_apic->io_apic_address,
		...
		case 2: { // Interrupt Source Override
			...
			if (num_overrides < 16) {
				irq_overrides[num_overrides].source_irq =
					override->irq_source;
				irq_overrides[num_overrides].global_irq =
			...
			struct ioapic_nonmaskable_interrupt_source *nmi =
				(struct ioapic_nonmaskable_interrupt_source
					 *)(entry +
					    sizeof(struct madt_record_header));
			log_debug(
				"[madt::parse_madt]: ioapic nmi: Source=%d, Global IRQ=%d\n",
			...
			struct local_apic_nonmaskable_interrupts *local_nmi =
				(struct local_apic_nonmaskable_interrupts
					 *)(entry +
					    sizeof(struct madt_record_header));
			log_debug(
				"[madt::parse_madt]: local apic nmi: Processor ID=%d, LINT=%d\n",
				local_nmi->acpi_processor_id,
				local_nmi->lint_number);
			break;
		}
			...
			struct local_apic_address_override *addr_override =
				(struct local_apic_address_override
					 *)(entry +
					    sizeof(struct madt_record_header));
			log_debug(
				"[madt::parse_madt]: local apic address override: 0x%x%x\n",
				addr_override
					->dqword_local_apic_physical_address[1],
				addr_override
					->dqword_local_apic_physical_address[0]);
			break;
		}
			...
			struct processor_local_x2apic *x2apic =
				(struct processor_local_x2apic
					 *)(entry +
					    sizeof(struct madt_record_header));
			log_debug(
				"[madt::parse_madt]: local x2APIC: ID=0x%x, Flags=0x%x, ACPI ID=%d\n",
				x2apic->processor_local_x2apic_id,
				x2apic->flags, x2apic->acpi_id);
			break;
		}
		default:
			log_warn(
				"[madt::parse_madt]: unknown MADT entry type: %d\n",
