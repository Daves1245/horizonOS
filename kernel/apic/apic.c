...
void configure_ioapic_irq_with_flags(uint8_t irq, uint8_t vector,
	...
	log_debug(
		"configuring IOAPIC: IRQ %d -> vector %d (dest=%d, flags=0x%x)\n",
...
void apic_send_eoi(void) {
	volatile uint32_t *apic_eoi =
		(volatile uint32_t *)(lapic_virt_base + APIC_EOI);
	*apic_eoi = 0;
}
...
void apic_timer_init(uint32_t hz) {
	volatile uint32_t *divide =
		(volatile uint32_t *)(lapic_virt_base + APIC_TIMER_DIVIDE);
	volatile uint32_t *initcnt =
		(volatile uint32_t *)(lapic_virt_base + APIC_TIMER_INITCNT);
	volatile uint32_t *currcnt =
		(volatile uint32_t *)(lapic_virt_base + APIC_TIMER_CURRCNT);
	volatile uint32_t *lvt =
		(volatile uint32_t *)(lapic_virt_base + APIC_LVT_TIMER);
