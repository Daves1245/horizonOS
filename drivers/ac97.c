...
int ac97_init(void) {
	...
	configure_ioapic_irq_with_flags(
		irq, vector, local_apic_id,
...
static void ready_codec(void) {
	...
	if (!timeout(10000, inl(nabmbar + AC97_GLOBAL_STATUS) &
				    AC97_GLOBAL_STATUS_CODEC_READY)) {
		abort();
	}
}
...
void ac97_setup_bdl(phys_addr_t audio_start, phys_addr_t audio_end) {
	if (audio_start > 0xFFFFFFFF || audio_end > 0xFFFFFFFF) {
		serial_write(
			"[ERROR]: ac97.c: audio data not in 32-bit addressable memory\n");
		hcf();
	}
	...
	while (!(inw(nabmbar + AC97_PCM_OUT_BASE +
		     AC97_CHANNEL_STATUS_REGISTER) &
	...
	for (i = 0; i < NUM_BDL_ENTRIES && audio_start < audio_end; i++) {
		...
		ring_buffer[i].flags =
			AC97_BDL_IOC; // ideally I suppose we'll only want to flag the last
...
void ac97_start_playback(void) {
	...
	outb(nabmbar + AC97_PCM_OUT_BASE + AC97_LAST_VALID_INDEX,
	     last_valid_index);

	// start DMA: set run/pause bit while preserving interrupt enable bits in the control register
	uint8_t cr = inb(nabmbar + AC97_PCM_OUT_BASE +
...
void ac97_debug_status(void) {
	...
	uint16_t sr =
		inw(nabmbar + AC97_PCM_OUT_BASE + AC97_CHANNEL_STATUS_REGISTER);
	uint8_t cr = inb(nabmbar + AC97_PCM_OUT_BASE +
			 AC97_CHANNEL_CONTROL_REGISTER);
	uint16_t picb =
		inw(nabmbar + AC97_PCM_OUT_BASE + AC97_POSITION_IN_CUR_BUFFER);
	...
	if (sr & AC97_STATUS_REGISTER_DMA_CONTROLLER_HALTED) {
		serial_write(
			"[DEBUG]: ac97_debug_status: -> DCH set (DMA halted)\n");
	} else {
		serial_write(
			"[DEBUG]: ac97_debug_status: -> DCH clear (DMA running)\n");
	}
}
...
static void ac97_irq_handler(struct interrupt_context *regs) {
	...
	int complete = status &
		       AC97_STATUS_REGISTER_BUFFER_COMPLETION_INTERRUPT_STATUS;
	...
	outw(nabmbar + AC97_PCM_OUT_BASE + AC97_CHANNEL_STATUS_REGISTER,
	     status);
	...
	if (!complete) {
		...
		int lvbci =
			status &
			AC97_STATUS_REGISTER_LAST_VALID_BUFFER_COMPLETION_INTERRUPT;
		if (lvbci) {
			serial_write(
				"[DEBUG]: [ac97.c]: LVBCI (last valid buffer completion interrupt)\n");
		} else {
			serial_write(
				"[DEBUG]: [ac97.c]: spurious interrupt (no BCIS or LVBCI)\n");
		}
		apic_send_eoi();
		return;
	}
